    ...
    def _input_terminal_disposition(
        ...
        if not turns.is_latest(pending.turn_id, pending.turn_revision):
            if turns.is_current_turn(pending.turn_id):
                ...
                return "discard"
            # A newer turn replaced this one, so its user item is final. Keep
            # routing state only while its late final can still arrive.
            if not turns.is_superseded(pending.turn_id, pending.turn_revision):
                pending.input_closed = True
            return "publish"
